...
{
	/* if the las operation was a write */
	...
	/* if the last operation was a read one */
		...
		/* character that was put in the buffer but no read */
...
size_t so_fread(void *ptr, size_t size, size_t nmemb, SO_FILE *stream)
{
	...
	while ((aux < nmemb * size) && stream->error == 0) {
		...
		/* reading or the end of file stop copying bytes */
...
/**
 * @brief This function reads bytes from the file and stores them in a internal
 * buffer
 *
 * @param stream the SO_FILE data structure
 * @return -1 - if there was an error or we reached EOF
 *         first character not "read" but wrote in the buffer - if everything
 * was ok
 */
 ...
 * buffer of the SO_FILE structure to be later wrote in the file
 *
...
int so_fputc(int c, SO_FILE *stream)
{
	...
	/* if we reached the ened of the buffer, flush it in the file */
...
SO_FILE *so_popen(const char *command, const char *type)
{
	...
	/* create the pipe used for communication between the child and parrent
	 */
	/* proccess */
	if (pipe(channel)) {
